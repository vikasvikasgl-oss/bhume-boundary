...
def run_test_scaling(village_dir: str, cap_factor: float = 12.0, fine_range_factor: float = 2.2):
    ...
    # Capping distance increased to prevent saturation
    cap_dist = float(cap_factor * pixel_size)
    control_cost_thresh = float(2.3 * pixel_size)
    improvement_thresh = float(0.6 * pixel_size)
    min_contrast_thresh = float(0.5 * pixel_size)
    
    # Scale anchor cost filter with pixel size
    max_anchor_cost = float(3.3 * pixel_size)
    ...
    for pn, res in coarse_results.items():
        if abs(res['dx']) >= 14.5 or abs(res['dy']) >= 14.5:
            continue
        if res['cost'] < max_anchor_cost and res['contrast'] > min_contrast_thresh and 0.85 <= res['area_ratio'] <= 1.15 and res['n_pts'] >= 15:
    ...
    for pn, row in plots_3857.iterrows():
        ...
        fine_search_range = float(fine_range_factor * pixel_size)
...
print("=== Running Scaling Test (cap_factor=12.0, fine_range=2.2) ===")
run_test_scaling('data/12429_malatavadi_chandgad_kolhapur', cap_factor=12.0, fine_range_factor=2.2)
run_test_scaling('data/34855_vadnerbhairav_chandavad_nashik', cap_factor=12.0, fine_range_factor=2.2)
