...
def run_isolated_test(village_dir: str):
    ...
    # Improvement threshold adjusted slightly to 0.55 * pixel_size
    improvement_thresh = float(0.55 * pixel_size)
    ...
    for pn, row in plots_3857.iterrows():
        ...
        # Base fine search range
        fine_search_range = float(2.2 * pixel_size)
        
        # Expand window for isolated plots (where prior is uncertain)
        dist_factor = priors[pn]['min_dist_anchor'] / 150.0
        if dist_factor > 1.0:
            fine_search_range *= min(3.0, dist_factor * 2.0)
...
print("=== Running Isolated Scaling Test ===")
run_isolated_test('data/12429_malatavadi_chandgad_kolhapur')
run_isolated_test('data/34855_vadnerbhairav_chandavad_nashik')
